...
def main():
    ...
    test_loss, test_acc, test_p, test_r, test_f1, counter = evaluate(
        model, test_iterator, criterion, TAG_PAD_IDX, TAG_UNK_IDX, UD_TAGS
    )
...
def categorical_accuracy(preds, y, tag_pad_idx, tag_unk_idx, UD_TAGS=None):
    """
    ...
    ret = correct.float().sum(), y[non_pad_elements].shape[0]
    ...
    if UD_TAGS != None:
        ...
        for tag in UD_TAGS.vocab.freqs.keys():
            ...
            if tag_idx == tag_pad_idx or tag_idx == tag_unk_idx:
                continue
            y_tag_elements = torch.nonzero(y == tag_idx)
            pred_tag_elements = torch.nonzero(max_preds == tag_idx)
            correct_tag_counter[tag] = max_preds[y_tag_elements].squeeze(1).eq(y[y_tag_elements]).float().sum().item()
            precision_denomin_counter[tag] = pred_tag_elements.shape[0]
            recall_denomin_counter[tag] = y_tag_elements.shape[0]
...
def evaluate(model, iterator, criterion, tag_pad_idx, tag_unk_idx, UD_TAGS=None):
        ...
        for batch in iterator:
            ...
            if UD_TAGS != None:
                correct, n_labels, c_counter, p_counter, r_counter = categorical_accuracy(
                    predictions, tags, tag_pad_idx, tag_unk_idx, UD_TAGS
                )
