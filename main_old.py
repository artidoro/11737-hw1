...
from model_old import BiLSTMPOSTagger
...
def main():
    ...
    train_data, valid_data, test_data = datasets.UDPOS.splits(
        ...
        train="ml-{}-ud-train.conll{}".format(args.lang, ext),
        validation="ml-{}-ud-dev.conll{}".format(args.lang, ext),
        test="ml-{}-ud-test.conll{}".format(args.lang, ext),
    ...
    if UD_TAGS != None:
        ...
        for tag in UD_TAGS.vocab.freqs.keys():
            ...
            if tag in precision_denomin_counter and tag in recall_denomin_counter and precision[tag] != 0 and recall[tag] != 0:
                ...
                f1[tag] = -0.01
        print(epoch_loss,  len(iterator), epoch_correct,  epoch_n_label, precision, recall, f1, recall_denomin_counter)
